    ...
    <body>        
        ...
        <div id="wrapperContent">
            ...
            <section id="LoginRegisterBox">
                <h2 id="h2Form">Please Enter Password Details</h2>
                <form name="registerCustomer" id="formValidation" method="post" action="UserActionServlet" class="minimal">
                    <label for="Password">
                        <p id="pForm">Old Password</p>
                        <input type="password" minlength="6" maxlength="30" name="oldPassword" id="password" placeholder="Password must contain 1 uppercase, lowercase and number" pattern="(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$" required="required" title="Password must contain 1 uppercase, lowercase and number"/>
                    </label>
                    <label for="Retype-Password">
                        <p id="pForm">New Password</p>
                        <input type="password" minlength="6" maxlength="30" name="newPassword" id="password" placeholder="Password must contain 1 uppercase, lowercase and number" pattern="(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$" required="required" title="Password must contain 1 uppercase, lowercase and number"/>
                    </label><label for="Password">
                        <p id="pForm">Retype-New-Password</p>
                        <input type="password" minlength="6" maxlength="30" name="retypeNewPassword" id="password" placeholder="Password must contain 1 uppercase, lowercase and number" pattern="(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$" required="required" title="Password must contain 1 uppercase, lowercase and number"/>
                    </label>
                    <input type="hidden" name="action" value="editPassword" />
                    <button type="submit" value="editPassword" class="button button--inline-block button--medium btn" id="btn">Update</button>
                </form>
            </section>
        </div>
