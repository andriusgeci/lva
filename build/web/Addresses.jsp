
<%@page import="DAO.AddressDao"%>
<%@page import="Business.Address"%>
<%@page import="java.util.HashMap"%>
<%@page import="Business.Movie"%>
<%@page import="DAO.MovieDao"%>
<%@page import="Business.User"%>
<%@page import="java.util.ArrayList"%>
<!DOCTYPE html>
<html lang="en">
    <head>
        <title>Addresses</title>
        <meta charset="UTF-8" />
        <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1"> 
        <meta name="viewport" content="width=device-width, initial-scale=1.0"> 
        <link rel="stylesheet" type="text/css" href="css/shop.css" />
        <link href='http://fonts.googleapis.com/css?family=Josefin+Slab' rel='stylesheet' type='text/css' />
        <link href="https://fonts.googleapis.com/icon?family=Material+Icons"rel="stylesheet">
        <link rel="stylesheet" type="text/css" href="css/admin.css" />
    </head>
    <body>  
        <%
            HashMap<Movie, Integer> cart = (HashMap) session.getAttribute("cart");
            User c = (User) session.getAttribute("user");
            AddressDao adr = new AddressDao();
            ArrayList<Address> addresses = adr.findAddressByUserId(c.getUserId());
        %>
        <div id="header">
            <div id="wrapper">
                <%  if (c != null) {
                %> 
                <a href="userProfile.jsp">
                    <strong>Welcome: </strong> <%=c.getFirstName()%> <%=c.getLastName()%>
                </a>
                <% } else {%>
                <a>
                    <strong>Welcome: </strong> Guest, please Login or Register.
                </a>
                <% }%>
                <span class="right">   
                    <%
                        if (c == null) {
                    %> 
                    <a href="register.jsp"><i id="headerIcons" class="material-icons">create</i>Register</a>
                    <a href="login.jsp"><i id="headerIcons" class="material-icons">exit_to_app</i>Login</a>
                    <%} else{%>
                    <%
                        if (c.isAdmin() == true) {
                    %> 
                    <a href="admin.jsp"> <i id="headerIcons" class="material-icons">settings</i>Admin</a>
                    <%}%>
                    <a href="userProfile.jsp"> <i id="headerIcons" class="material-icons">account_circle</i>My Profile</a>
                    <a href="logout.jsp"><i id="headerIcons" class="material-icons">exit_to_app</i>Logout</a>
                    <% }%>
                    <% if (cart != null) {%>
                    <a href="shoppingCart.jsp"><i  id="headerIcons" class="material-icons">shopping_cart</i><%=cart.size()%></a>
                    <% } else { %>
                    <a href="shoppingCart.jsp"><i  id="headerIcons" class="material-icons">shopping_cart</i>0</a>
                    <% }%>
                </span>
            </div>
        </div>
        <div id="wrapper">
            <div id="navigation">
                <ul>
                    <li><a href="index.jsp">Home</a></li>
                    <li><a href="shop.jsp">Shop</a></li>                
                    <li><a href="MovieCharts.jsp">Movie Charts</a></li>
                    <li><a href="NewReleases.jsp">New Releases</a></li>
                    <li><a href="ComingSoon.jsp">Coming Soon</a></li>
                </ul> 
            </div>
        </div>
        <div id="grow">
            <div id="wrapper">
                <h1>My Addresses</h1>
            </div>
        </div>  
        <div id="wrapperContent">
            <div id="navigation">
                <ul>
                    <li><a href="userProfile.jsp">Personal Information</a></li>
                    <li><a href="Addresses.jsp">Addresses</a></li>
                    <li><a href="MyReviews.jsp">My Reviews</a></li>
                    <li><a href="OrderHistory.jsp">Order History</a></li>   
                </ul> 
            </div>
            <section id="AddressBox">
                <hr>
                <b>Don't Have Address Yet? Click the button below to Add One.</b><br>
                <hr>


                <a href='addAddress.jsp'>
                    <button type="submit" value="addAddress" class="button button--inline-block button--medium btn" id="btn">Add Address</button>
                </a>   
                <hr>

            </section>
            <%
                for (int i = 0; i < addresses.size(); i++) {

            %>
            <section id="AddressBox">
                <hr>
                <%                    if (addresses.get(i).isPrimaryAddress() == true) {
                %>
                <b>Primary/Shipping Address</b><br>
                <%} else {%>
                <b>Secondary Address</b><br>
                <%}%>
                <hr>
                <b>Address Line 1: </b><%=addresses.get(i).getAddress1()%><br>
                <% if (addresses.get(i).getAddress2() == null || addresses.get(i).getAddress2().isEmpty()) {%>
                <b>Address Line 2: </b>No Address Line 2<br>
                <%} else {%>
                <b>Address Line 2: </b><%=addresses.get(i).getAddress2()%><br>
                <%}%>
                <b>City:  </b><%=addresses.get(i).getCity()%><br>
                <b>County: </b><%=addresses.get(i).getCounty()%><br>
                <% if (addresses.get(i).getPostalCode() == null || addresses.get(i).getPostalCode().isEmpty()) {%>
                <b>Postal Code: </b>No Postal Code<br>
                <%} else {%>
                <b>Postal Code: </b><%=addresses.get(i).getPostalCode()%><br>
                <%}%>
                <b>Country: </b><%=addresses.get(i).getCountry()%><br>
                <hr>
                <%if(addresses.get(i).isPrimaryAddress() == true) {%>
                <form name="updateAddress" method="post" action="UserActionServlet" class="minimal">
                    <input type="hidden" name="action" value="updateAddress" />
                    <input type="hidden" name="param" value="<%=Integer.toString(addresses.get(i).getAddressID())%>" /> 
                    <button type="submit" value="updateAddress" class="button button--inline-block button--medium btn" id="btn">Edit</button>
                </form>
                <%}else{%>
                <form name="updateAddress" method="post" action="UserActionServlet" class="minimal">
                    <input type="hidden" name="action" value="updateAddress" />
                    <input type="hidden" name="param" value="<%=Integer.toString(addresses.get(i).getAddressID())%>" /> 
                    <button type="submit" value="updateAddress" class="button button--inline-block button--medium btn" id="btn">Edit</button>
                </form>
                <form name="deleteAddress" method="post" action="UserActionServlet" class="minimal">
                    <input type="hidden" name="delete" value="<%=Integer.toString(addresses.get(i).getAddressID())%>" /> 
                    <input type="hidden" name="action" value="deleteAddress" />
                    <button type="submit" name="action" value="deleteAddress" class="button button--medium btn" id="btn">Delete</button>
                </form> 
                <%}%>    
            </section>
            <%
                }
            %>
        </div>



        <div id="footer1">
            <p>Copyright &copy; 2016 LVA Movies All Rights Reserved.</p>
            <a href="about.jsp">About </a><a href="https://www.facebook.com/profile.php?id=100011449903415"> Facebook</a><a href="https://twitter.com/lvamoviess"> Twitter</a>
        </div>   
    </body>
</html>
